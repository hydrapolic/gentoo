...
IUSE="api debug doc +hvm +ipxe lzma ocaml ovmf pygrub python +qemu +rombios screen selinux sdl static-libs system-ipxe system-qemu system-seabios systemd zstd"
...
src_prepare() {
	...
	if [[ -v XEN_GENTOO_PATCHES_DIR ]]; then
		rm -v "${WORKDIR}"/xen-gentoo-patches-4.20.1-gentoo-patchset-0/xen-tools-4.4.1-tinfo.patch || die
		eapply "${XEN_GENTOO_PATCHES_DIR}"
	fi

	eapply "${FILESDIR}"/xen-tools-4.21.1-tinfo.patch
	...
	if ! use pygrub; then
		sed -e '/^SUBDIRS-\$(CONFIG_PYGRUB) += pygrub/d' -i tools/Makefile || die
	fi
	...
	# Make sure the script is not used
	rm scripts/git-checkout.sh || die
